...
static LIST_HEAD_LINUX(gref_list);
// static DEFINE_MUTEX(gref_mutex);
static mutex_t gref_mutex;
...
/* ---------------------------ADDED WRAPPERS */
static void evtchn_put(int ev)
{

}

static int evtchn_get(evtchn_port_t ev)
{
	return 0;
}
	...
	uint32_t *gref_ids, struct gntalloc_file_private_data *priv)
{
	int i, rc, readonly;
	grant_ref_t r_ref;
	LIST_HEAD_LINUX(queue_gref);
	LIST_HEAD_LINUX(queue_file);
	struct gntalloc_gref *gref, *next;

	readonly = !(op->flags & GNTALLOC_FLAG_WRITABLE);
	for (i = 0; i < op->count; i++) {
		gref = (struct gntalloc_gref *)kzalloc(sizeof(*gref), GFP_KERNEL);
		if (!gref) {
			rc = -ENOMEM;
			goto undo;
		}
		...
		rc = gnttab_grant_foreign_access(op->domid,
						 ((uint64_t)gref->page) >> PAGE_SHIFT,
						 readonly, &r_ref);
		if (rc < 0)
			goto undo;
		gref_ids[i] = gref->gref_id = r_ref;
	}
 ...
 * -------------------------------------
 */
static int gntalloc_open(struct device *dev, int __unused _pad)
{
	struct gntalloc_file_private_data *priv;

	priv = (gntalloc_file_private_data *)kzalloc(sizeof(*priv), GFP_KERNEL);
	if (!priv)
		goto out_nomem;
...
static int gntalloc_release(struct device *dev)
{
	struct gntalloc_file_private_data *priv = (struct gntalloc_file_private_data *)dev->private_data;
	struct gntalloc_gref *gref;
		...
		struct ioctl_gntalloc_alloc_gref *arg)
{
	...
	gref_ids = (uint32_t *)kcalloc(op.count, sizeof(gref_ids[0]), GFP_KERNEL);
	if (!gref_ids) {
		rc = -ENOMEM;
		goto out;
	}
...
static int gntalloc_ioctl(struct device *dev, u_long cmd, void *arg)
{
	struct gntalloc_file_private_data *priv = (struct gntalloc_file_private_data *)dev->private_data;

	switch (cmd) {
	case IOCTL_GNTALLOC_ALLOC_GREF:
		return gntalloc_ioctl_alloc(priv, (struct ioctl_gntalloc_alloc_gref *)arg);

	case IOCTL_GNTALLOC_DEALLOC_GREF:
		return gntalloc_ioctl_dealloc(priv, (void *)arg);

	case IOCTL_GNTALLOC_SET_UNMAP_NOTIFY:
		return gntalloc_ioctl_unmap_notify(priv, (void *)arg);
...
/* Because mmap was splitted into two, this struct is used to temporarily save information
 * between mmap and fault handlers
 */
struct mmap_temp_store *mmap_store_head;

static int gntalloc_mmap(struct device *dev, uintptr_t start, uintptr_t end, uintptr_t off, unsigned write, unsigned share)
{
	struct gntalloc_file_private_data *priv = (struct gntalloc_file_private_data *)dev->private_data;
	...
	printf("gntalloc_mmap: priv: %p, off: %x, count: %d\n", priv, off, count);
	gref = find_grefs(priv, off << PAGE_SHIFT, count);
	if (gref == NULL) {
		rv = -ENOENT;
		printf("%s: Could not find grant reference", __func__);
		// kfree(vm_priv);
		goto out_unlock;
	}
	...
	for (i = 0; i < count; i++) {
		...
		// if (rv)
		// 	goto out_unlock;
		mmap_temp_store_add(mmap_store_head, start, end, gref->page);
...
static int gntalloc_on_fault(struct device *dev, uintptr_t start, uintptr_t end, 
	uintptr_t off, unsigned write, unsigned share, void **addr_save)
{
	struct mmap_temp_store *t = mmap_temp_store_search(mmap_store_head, start, end);
	if (!t) return 1;
	int count = (off & 0x1000) >> PAGE_SHIFT;
	*addr_save = t->pages[count];
	return 0;
}
...
// 	.fops	= &gntalloc_fops,
// };
struct devops gntalloc_device_devops = {
    gntalloc_open,
    gntalloc_release,
    no_read,
    no_write,
...
struct vmaops gntalloc_device_vmaops = {
	gntalloc_on_fault,
};
