...
/* ---------------------VM FUNCTION WRAPPERS----------------------- */
#ifdef __cplusplus
#endif
/* ---------------------END VM FUNCTION WRAPPERS------------------- */
...
// TODO: put it somewhere in struct driver private data
static void *_private_data;
...
static int
devfs_set_cdevpriv(void *priv, d_priv_dtor_t __unused dtor)
{
	_private_data = priv;
	return 0;
}
    ...
    uint64_t file_offset)
{
	...
	offset = (struct file_offset_struct *)malloc(sizeof(*offset), M_GNTDEV, M_WAITOK | M_ZERO);
...
gntdev_alloc_gref(struct ioctl_gntdev_alloc_gref *arg)
{
	...
	/* Allocate grefs. */
	grefs = (struct gntdev_gref *)malloc(sizeof(*grefs) * arg->count, M_GNTDEV, M_WAITOK);

	for (i = 0; i < arg->count; i++) {
		grefs[i].file_index = file_offset + i * PAGE_SIZE;
		grefs[i].gref_id = GRANT_REF_INVALID;
		grefs[i].notify = NULL;
		grefs[i].page = alloc_page();
		if (grefs[i].page == NULL) {
			debugf("Page allocation failed.");
			error = ENOMEM;
			break;
		}
		memset(grefs[i].page, 0, PAGE_SIZE);

		error = gnttab_grant_foreign_access(arg->domid,
			(virt_to_phys(grefs[i].page) >> PAGE_SHIFT),
			readonly, &grefs[i].gref_id);
		if (error != 0) {
			debugf("Grant Table Hypercall failed.");
			break;
		}
	}
